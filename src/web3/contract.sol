...
contract Lure {
    // Constants
    string public constant MSG_IS_A_LURE_PLAN =
        "Just a LurePlan can call to this function.";
    string public constant MSG_NO_FUNDS =
    ...
    // Variables
    mapping(address => uint256) public busdOf;
    mapping(address => uint256) public usdtOf;
    mapping(address => uint256) public ambarOf;
    mapping(address => uint256) public bnbOf;

    // Public Methods

    // Withdraws
    function withdraw_busd(uint256 amount) public {
        require(busdOf[msg.sender] >= amount, MSG_WITHDRAW_FAIL);
        require(get_amount_of_our_busd() >= amount, MSG_NO_FUNDS);
        require(BUSD.transfer(msg.sender, amount), MSG_BUSD_FAIL);

        busdOf[msg.sender] -= amount;
    }

    function withdraw_usdt(uint256 amount) public {
        require(usdtOf[msg.sender] >= amount, MSG_WITHDRAW_FAIL);
        require(get_amount_of_our_usdt() >= amount, MSG_NO_FUNDS);
        require(USDT.transfer(msg.sender, amount), MSG_USDT_FAIL);

        usdtOf[msg.sender] -= amount;
    }

    function withdraw_ambar(uint256 amount) public {
        require(ambarOf[msg.sender] >= amount, MSG_WITHDRAW_FAIL);
        require(get_amount_of_our_ambar() >= amount, MSG_NO_FUNDS);
        require(AMBAR.transfer(msg.sender, amount), MSG_AMBAR_FAIL);

        ambarOf[msg.sender] -= amount;
    }

    function withdraw_bnb(uint256 amount) public {
        require(bnbOf[msg.sender] >= amount, MSG_WITHDRAW_FAIL);
        require(get_amount_of_our_bnb() >= amount, MSG_NO_FUNDS);
        payable(msg.sender).transfer(amount);

        bnbOf[msg.sender] -= amount;
    }

    // Deposits
    function deposit_busd(uint256 amount) public {
        require(
            BUSD.transferFrom(msg.sender, ownerWallet, amount),
            MSG_BUSD_FAIL
        );
        busdOf[msg.sender] += amount;
    }
    ...
    function deposit_usdt(uint256 amount) public {
        require(
            USDT.transferFrom(msg.sender, ownerWallet, amount),
            MSG_USDT_FAIL
        );
        usdtOf[msg.sender] += amount;
    }

    function deposit_ambar(uint256 amount) public {
        require(
            AMBAR.transferFrom(msg.sender, ownerWallet, amount),
            MSG_AMBAR_FAIL
        );
        ambarOf[msg.sender] += amount;
    }
    ...
    function deposit_bnb() public payable {
        bnbOf[msg.sender] += msg.value;
    }

    // Begginers Investment
    function invest_begginer_busd(uint256 amount) public {
        require(busdOf[msg.sender] >= amount, MSG_NO_FUNDS_USER);
        begginer.invest_erc20(amount, msg.sender, address(BUSD));
        busdOf[msg.sender] -= amount;
    }

    function invest_begginer_usdt(uint256 amount) public {
        require(usdtOf[msg.sender] >= amount, MSG_NO_FUNDS_USER);
        begginer.invest_erc20(amount, msg.sender, address(USDT));
        usdtOf[msg.sender] -= amount;
    }

    function invest_begginer_ambar(uint256 amount) public {
        require(ambarOf[msg.sender] >= amount, MSG_NO_FUNDS_USER);
        begginer.invest_erc20(amount, msg.sender, address(AMBAR));
        ambarOf[msg.sender] -= amount;
    }

    function invest_begginer_bnb(uint256 amount) public {
        require(bnbOf[msg.sender] >= amount, MSG_NO_FUNDS_USER);
        begginer.invest_bnb(amount, msg.sender);
        bnbOf[msg.sender] -= amount;
    }

    // Standard Investment
    function invest_standard_busd(uint256 amount) public {
        require(busdOf[msg.sender] >= amount, MSG_NO_FUNDS_USER);
        standard.invest_erc20(amount, msg.sender, address(BUSD));
        busdOf[msg.sender] -= amount;
    }

    function invest_standard_usdt(uint256 amount) public {
        require(usdtOf[msg.sender] >= amount, MSG_NO_FUNDS_USER);
        standard.invest_erc20(amount, msg.sender, address(USDT));
        usdtOf[msg.sender] -= amount;
    }

    function invest_standard_ambar(uint256 amount) public {
        require(ambarOf[msg.sender] >= amount, MSG_NO_FUNDS_USER);
        standard.invest_erc20(amount, msg.sender, address(AMBAR));
        ambarOf[msg.sender] -= amount;
    }

    function invest_standard_bnb(uint256 amount) public {
        require(bnbOf[msg.sender] >= amount, MSG_NO_FUNDS_USER);
        standard.invest_bnb(amount, msg.sender);
        bnbOf[msg.sender] -= amount;
    }

    // Expert Investment
    function invest_expert_busd(uint256 amount) public {
        require(busdOf[msg.sender] >= amount, MSG_NO_FUNDS_USER);
        expert.invest_erc20(amount, msg.sender, address(BUSD));
        busdOf[msg.sender] -= amount;
    }

    function invest_expert_usdt(uint256 amount) public {
        require(usdtOf[msg.sender] >= amount, MSG_NO_FUNDS_USER);
        expert.invest_erc20(amount, msg.sender, address(USDT));
        usdtOf[msg.sender] -= amount;
    }

    function invest_expert_ambar(uint256 amount) public {
        require(ambarOf[msg.sender] >= amount, MSG_NO_FUNDS_USER);
        expert.invest_erc20(amount, msg.sender, address(AMBAR));
        ambarOf[msg.sender] -= amount;
    }

    function invest_expert_bnb(uint256 amount) public {
        require(bnbOf[msg.sender] >= amount, MSG_NO_FUNDS_USER);
        expert.invest_bnb(amount, msg.sender);
        bnbOf[msg.sender] -= amount;
    }

    // Business Investment
    function invest_business_busd(uint256 amount) public {
        require(busdOf[msg.sender] >= amount, MSG_NO_FUNDS_USER);
        business.invest_erc20(amount, msg.sender, address(BUSD));
        busdOf[msg.sender] -= amount;
    }

    function invest_business_usdt(uint256 amount) public {
        require(usdtOf[msg.sender] >= amount, MSG_NO_FUNDS_USER);
        business.invest_erc20(amount, msg.sender, address(USDT));
        usdtOf[msg.sender] -= amount;
    }

    function invest_business_ambar(uint256 amount) public {
        require(ambarOf[msg.sender] >= amount, MSG_NO_FUNDS_USER);
        business.invest_erc20(amount, msg.sender, address(AMBAR));
        ambarOf[msg.sender] -= amount;
    }

    function invest_business_bnb(uint256 amount) public {
        require(bnbOf[msg.sender] >= amount, MSG_NO_FUNDS_USER);
        business.invest_bnb(amount, msg.sender);
        bnbOf[msg.sender] -= amount;
    }

    // Premium Investment
    function invest_premium_busd(uint256 amount) public {
        require(busdOf[msg.sender] >= amount, MSG_NO_FUNDS_USER);
        premium.invest_erc20(amount, msg.sender, address(BUSD));
        busdOf[msg.sender] -= amount;
    }

    function invest_premium_usdt(uint256 amount) public {
        require(usdtOf[msg.sender] >= amount, MSG_NO_FUNDS_USER);
        premium.invest_erc20(amount, msg.sender, address(USDT));
        usdtOf[msg.sender] -= amount;
    }

    function invest_premium_ambar(uint256 amount) public {
        require(ambarOf[msg.sender] >= amount, MSG_NO_FUNDS_USER);
        premium.invest_erc20(amount, msg.sender, address(AMBAR));
        ambarOf[msg.sender] -= amount;
    }

    function invest_premium_bnb(uint256 amount) public {
        require(bnbOf[msg.sender] >= amount, MSG_NO_FUNDS_USER);
        premium.invest_bnb(amount, msg.sender);
        bnbOf[msg.sender] -= amount;
    }

    // Executive Investment
    function invest_executive_busd(uint256 amount) public {
        require(busdOf[msg.sender] >= amount, MSG_NO_FUNDS_USER);
        executive.invest_erc20(amount, msg.sender, address(BUSD));
        busdOf[msg.sender] -= amount;
    }

    function invest_executive_usdt(uint256 amount) public {
        require(usdtOf[msg.sender] >= amount, MSG_NO_FUNDS_USER);
        executive.invest_erc20(amount, msg.sender, address(USDT));
        usdtOf[msg.sender] -= amount;
    }

    function invest_executive_ambar(uint256 amount) public {
        require(ambarOf[msg.sender] >= amount, MSG_NO_FUNDS_USER);
        executive.invest_erc20(amount, msg.sender, address(AMBAR));
        ambarOf[msg.sender] -= amount;
    }

    function invest_executive_bnb(uint256 amount) public {
        require(bnbOf[msg.sender] >= amount, MSG_NO_FUNDS_USER);
        executive.invest_bnb(amount, msg.sender);
        bnbOf[msg.sender] -= amount;
    }
    ...
    function get_amount_of_our_bnb() public view returns (uint256) {
        return address(this).balance;
    }

    // Communication Methods
    function increase_busd_of(address wallet, uint256 amount) public {
        require(is_a_lure_plan(msg.sender), MSG_IS_A_LURE_PLAN);
        busdOf[wallet] += amount;
    }

    function increase_usdt_of(address wallet, uint256 amount) public {
        require(is_a_lure_plan(msg.sender), MSG_IS_A_LURE_PLAN);
        usdtOf[wallet] += amount;
    }

    function increase_ambar_of(address wallet, uint256 amount) public {
        require(is_a_lure_plan(msg.sender), MSG_IS_A_LURE_PLAN);
        ambarOf[wallet] += amount;
    }

    function increase_bnb_of(address wallet, uint256 amount) public {
        require(is_a_lure_plan(msg.sender), MSG_IS_A_LURE_PLAN);
        bnbOf[wallet] += amount;
    }

    function set_begginer_plan(address plan) public {
        require(msg.sender == developmentWallet, MSG_DEV_WALLET);
        begginer = LurePlan(plan);
    }

    function set_standard_plan(address plan) public {
        require(msg.sender == developmentWallet, MSG_DEV_WALLET);
        standard = LurePlan(plan);
    }

    function set_expert_plan(address plan) public {
        require(msg.sender == developmentWallet, MSG_DEV_WALLET);
        expert = LurePlan(plan);
    }

    function set_business_plan(address plan) public {
        require(msg.sender == developmentWallet, MSG_DEV_WALLET);
        business = LurePlan(plan);
    }

    function set_premium_plan(address plan) public {
        require(msg.sender == developmentWallet, MSG_DEV_WALLET);
        premium = LurePlan(plan);
    }

    function set_executive_plan(address plan) public {
        require(msg.sender == developmentWallet, MSG_DEV_WALLET);
        executive = LurePlan(plan);
    }

    // Private Methods
    function is_a_lure_plan(address wallet) private view returns (bool) {
        return (wallet == address(begginer) ||
            wallet == address(standard) ||
            wallet == address(expert) ||
